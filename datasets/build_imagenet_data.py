...
tf.app.flags.DEFINE_string('train_directory', 'trains', 'Training data directory')
tf.app.flags.DEFINE_string('validation_directory', 'validates', 'Validation data directory')
tf.app.flags.DEFINE_string('output_directory', 'outputs', 'Output data directory')
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames, synsets, labels, humans, bboxes, num_shards):
    ...
    :param ranges:list of pairs of integers specifying ranges of each batches to
      analyze in parallel.
    ...
    :param humans:list of strings; each string is a human-readable label
    :param bboxes:list of bounding boxes for each image. Note that each entry in this
...
def _find_human_readable_labels(synsets, synset_to_human):
    ...
    :param synsets: list of strings; each string is a unique WordNet ID.
    :param synset_to_human: dict of synset to human labels, e.g.,
      'n02119022' --> 'red fox, Vulpes vulpes'
    :return:  List of human-readable strings corresponding to each synset.
    """
    humans = []
    for s in synsets:
...
def _find_image_bounding_boxes(filenames, image_to_bboxes):
    ...
    :param filenames: list of strings; each string is a path to an image file.
    :param image_to_bboxes: dictionary mapping image file names to a list of
      bounding boxes. This list contains 0+ bounding boxes.
    :return:  List of bounding boxes for each image. Note that each entry in this
              list might contain from 0+ entries corresponding to the number of bounding
              box annotations for the image.
    """
    num_image_bbox = 0
...
def _process_dataset(name, directory, num_shards, synset_to_human, image_to_bboxes):
    ...
    :param name: string, unique identifier specifying the data set.
    :param directory: string, root path to the data set.
    :param num_shards: integer number of shards for this data set.
    :param synset_to_human: dict of synset to human labels, e.g.,
      'n02119022' --> 'red fox, Vulpes vulpes'
    :param image_to_bboxes:  dictionary mapping image file names to a list of
...
def _build_synset_lookup(imagenet_metadata_file):
    ...
    :param imagenet_metadata_file:  string, path to file containing mapping from synset to human-readable label.
         Assumes each line of the file looks like:
         n02119247    black fox
         n02119359    silver fox
         n02119477    red fox, Vulpes fulva
         where each line corresponds to a unique mapping. Note that each line is
         formatted as <synset>\t<human readable label>.
    :return:  Dictionary of synset to human labels, such as:
              'n02119022' --> 'red fox, Vulpes vulpes'
    """
...
def _build_bounding_box_lookup(bounding_box_file):
    ...
    :param bounding_box_file: string, path to file with bounding boxes annotations.
      Assumes each line of the file looks like:
      n00007846_64193.JPEG,0.0060,0.2620,0.7545,0.9940
       where each line corresponds to one bounding box annotation associated
      with an image. Each line can be parsed as:
