...
def _crop(image, offset_height, offset_width, crop_height, crop_width):
    '''
    Crops the given image using the provided offsets and sizes. Note that the method doesn't assume we know
    the input image size but it does
    ...
    with tf.control_dependencies([rank_assertion]):  # 在执行下文的操作前，要先执行rank_assertion操作
        cropped_shape = tf.stack([crop_height, crop_width, original_shape[2]])

    size_assertion = tf.Assert(tf.logical_and(tf.greater(original_shape[0], crop_height),
                                              tf.greater(original_shape[1], crop_width)),
                               ['Crop size greater than the image size'])
    offsets = tf.to_int32(tf.stack([offset_height, offset_width, 0]))
    # Use tf.slice instead of crop_to_bounding box as it accepts tensors to
    # define the crop size.
    with tf.control_dependencies(size_assertion):
        image = tf.slice(image, offsets, cropped_shape)  # 有可能会丢失第三个维度，所以会在后面reshape操作一下
    return tf.reshape(image, cropped_shape)


def _random_crop(image_list, crop_height, crop_width):
    """
    Crops the given list of images. The function applies the same crop to each image in the list. This can be
    effectively applied when there are multiple image inputs of the same
    dimension such as:
    image, depths, normals = _random_crop([image, depths, normals], 120, 150)
    :param image_list: a list of image tensors of the same dimension but possibly varying channel.
    :param crop_height: the new height.
    :param crop_width: the new width.
    :return:  the image_list with cropped images.
    Raises:
    ...
    for i in range(len(image_list)):
        ...
        rank_assert = tf.Assert(tf.equal(image_rank, 3), ['Wrong rank for tensor %s [expected] [actual]',
...
def _smallest_size_at_least(height, width, smallest_side):
  ...
  preserving the original aspect ratio.
    ...
    :param smallest_side: A python integer or scalar `Tensor` indicating the size of the smallest side after resize.
    :return: new_height: an int32 scalar tensor indicating the new height.
    ...
    scale = tf.cond(tf.greater_equal(height, width), lambda: smallest_side / width, lambda: smallest_side / height)
    new_height = tf.to_float(height * scale)
...
def _aspect_preserving_resize(image, smallest_side):
    ...
    height = shape[0]
    width = shape[1]
    new_width, new_height = _smallest_size_at_least(height, width, smallest_side)
...
def preprocess_for_train(image, output_height, output_width, resize_side_min = _RESIZE_SIDE_MIN,
                         resize_side_max = _RESIZE_SIDE_MAX):
    """
    processes the given image for training. Note that the actual resizing scale is sampled from
    ...
    :param resize_side_min: The lower bound for the smallest side of the image for
      aspect-preserving resizing.
    :param resize_side_max: The upper bound for the smallest side of the image for
      aspect-preserving resizing.
