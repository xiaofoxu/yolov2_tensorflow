...
class DeploymentConfigTest(tf.test.TestCase):
    ...
    def testDefaults(self):
        ...
        # print(deploy_config.clone_device(0))
    ...
    def testPS(self):
        ...
        self.assertDeviceEqual('/job:ps/device:CPU:0/task:0', '/job:ps/task:0/device:CPU:0')  # tf内部会对字符串重新分割组合成一样的顺序
    ...
    def testVariablesPS(self):
        ...
        with tf.device(deploy_config.variables_device()):
            ...
            b = tf.Variable(0)
            c = tf.no_op()
            d = slim.variable('hhhgf', [], caching_device=deploy_config.caching_device())
        self.assertDeviceEqual('/job:ps/device:CPU:0/task:0', '/job:ps/task:0/device:CPU:0')  # tf内部会对字符串重新分割组合成一样的顺序
    ...
    def testReplicasMultiGPUPS(self):
        deploy_config = model_deploy.DeploymentConfig(num_clones=2, num_replicas=2, num_ps_tasks=2)
...
def LogisticClassifier(inputs, labels, scope=None, reuse=None):
    with tf.variable_scope(scope, 'LogisticClassifier', [inputs, labels], reuse=reuse):
        # print("line--3", tf.get_variable_scope().original_name_scope)
        predictions = slim.fully_connected(inputs, 1, activation_fn=tf.sigmoid, scope='fully_connected')
        tf.losses.log_loss(predictions=predictions, labels=labels)
        return predictions
...
class CreatecloneTest(tf.test.TestCase):
    ...
    def testCreateMulticlone(self):
        ...
        with g.as_default():
            ...
            for i, clone in enumerate(clones):
                self.assertEqual(clone.outputs.op.name, 'clone_%s/BatchNormClassifier/fully_connected/Sigmoid' % i)
    ...
    def testCreateMultiCloneWithPS(self):
        ...
        with g.as_default():
            tf.set_random_seed(0)
            tf_input = tf.constant(self._inputs, dtype=tf.float32)
            tf_label = tf.constant(self._labels, dtype=tf.float32)

            model_fn = BatchNormClassifier
            clone_args = (tf_input, tf_label)
            deploy_config = model_deploy.DeploymentConfig(num_clones=2, num_ps_tasks=2)
            self.assertEqual(slim.get_variables(), [])
            clones = model_deploy.create_clones(deploy_config, model_fn, clone_args)
            self.assertEqual(len(slim.get_variables()), 5)
...
class OptimizeclonesTest(tf.test.TestCase):
    ...
    def testCreateSingleClone(self):
        ...
        with g.as_default():
            ...
            total_loss, grads_and_vars = model_deploy.optimize_clones(clones, optimizer)  # 前向传播
            self.assertEqual(len(grads_and_vars), len(tf.trainable_variables()))
            self.assertEqual(total_loss.op.name, 'total_loss')
            for g, v in grads_and_vars:
                # print('g = ', g.device)
...
class DeployTest(tf.test.TestCase):
    ...
    def testLocalTrainOp(self):
        ...
        with g.as_default():
            ...
            self.assertEqual(len(update_op), 4)
            for u in update_op:
                print(u.op.name)
            ...
            with tf.Session() as sess:
                ...
                moving_variance = tf.contrib.framework.get_variables_by_name('moving_variance')[0]
                ...
                self.assertAllClose(final_variance, [0.109375, 0.1875, 0.234375, 0.1875])
    ...
    def testNoSummariesOnGpu(self):
        with tf.Graph().as_default():
            ...
            model = model_deploy.deploy(deploy_config, model_fn=ModelFn, optimizer=tf.train.GradientDescentOptimizer(1.0))
