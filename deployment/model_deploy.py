...
Clone = collections.namedtuple('Clone', ['outputs',  # Whatever model_fn() returned
...
def create_clones(config, model_fn, args=None, kwargs=None):
    ...
    the scope and device used to created it in a namedtuple `Clone(outputs, scope, device)`
    ...
    :return:  A list of namedtuples `Clone`.
    """
    clones = []
    args = args or []
    kwargs = kwargs or {}
...
def _gather_clone_loss(clone, num_clones, regularization_losses):
    ...
    :param regularization_losses: Possibly empty list of regularization_losses to add to the clone losses.
    :return: A tensor for the total loss for the clone.  Can be None.
    """
    sum_loss = None  # The return value.
...
def _optimize_clone(optimizer, clone, num_clones, regularization_losses, **kwargs):
    ...
    :param optimizer: A tf.Optimizer  object.
    :param clone:  A Clone namedtuple.
    :param num_clones: The number of clones being deployed.
    :param regularization_losses: Possibly empty list of regularization_losses to add to the clone losses.
    :param kwargs: Dict of kwarg to pass to compute_gradients().
    :return:   A tuple (clone_loss, clone_grads_and_vars).
...
def optimize_clones(clones, optimizer, regularization_losses=None, **kwargs):
    ...
    :param regularization_losses:  regularization_losses: Optional list of regularization losses. If None it
     will gather them from tf.GraphKeys.REGULARIZATION_LOSSES. Pass `[]` to
     exclude them.
    :param kwargs: Optional list of keyword arguments to pass to `compute_gradients`.
    :return:
...
def deploy(config, model_fn, args=None, kwargs=None, optimizer=None, summarize_gradients=False):
    ...
    :param args: Optional list of arguments to pass to `model_fn`.
    :param kwargs: Optional list of keyword arguments to pass to `model_fn`.
    :param optimizer: Optional `Optimizer` object.  If passed the model is deployed for training with that optimizer.
    :param summarize_gradients: Whether or not add summaries to the gradients.
    :return: A `DeployedModel` namedtuple.
    """
    summaries = set(tf.get_collection(tf.GraphKeys.SUMMARIES))  # Gather initial summaries
    clones = create_clones(config, model_fn, args, kwargs)  # create clones
    first_clone = clones[0]

    # Gather update_ops from the first clone. These contain, for example,
    # the updates for the bach_norm variables created by model_fn.
    ...
    with tf.device(config.optimizer_device()):
        if optimizer:
            ...
            total_loss, clones_gradients = optimize_clones(clones, optimizer)  # Compute the gradients for the clones.

            if clones_gradients:
                if summarize_gradients:
                    summaries |= set(_add_gradients_summaries(clones_gradients))  # Add summaries to the gradients.
            ...
            if total_loss is not None:
                summaries.add(tf.summary.scalar('total_loss', total_loss))  # add total_loss to summary
...
def _sum_clones_gradients(clone_grads):
    ...
    :param clone_grads: A List of List of tuples (gradient, variable), one list per `Clone`.
    :return: List of tuples of (gradient, variable) where the gradient has been summed across all clones.
    """
    sum_grads = []
    for grad_and_vars in zip(*clone_grads):
        # Note that each grad_and_vars looks like the following:
        #   ((grad_var0_clone0, var0), ... (grad_var0_cloneN, var0))
        grads = []
...
def _add_gradients_summaries(grads_and_vars):
    """
    Add histogram summaries to gradients. Note: The summaries are also added to the SUMMARIES collection.
    :param grads_and_vars: A list of gradient to variable pairs (tuples).
    :return:  The _list_ of the added summaries for grads_and_vars.
    """
    summaries = []
...
class DeploymentConfig(object):
    ...
    def __init__(self,
        ...
        :param num_clones: Number of model clones to deploy in each replica.(not replica maybe a machine)
        :param clone_on_cpu: True if clones should be placed on CPU
        :param replica_id: Integer.  Index of the replica for which the model is
                            deployed.  Usually 0 for the chief replica.(The task id of this training instance)
        :param num_replicas: Number of replicas to use.
        :param num_ps_tasks: Number of tasks for the `ps` job. 0 to not use replicas.(The number of parameter servers)
                             If the value is 0, then the parameters are handled locally by the worker.'
                             (maybe one task corresponding one machine, 或者说是参数服务器需要做的工作的类别数)
    ...
    def caching_device(self):
        ...
        :return: A device string or None if the variables do not need to be cached
        """
        if self._num_ps_tasks > 0:
            return lambda op: op.device  # 返回一个lambda表达式
        else:
            return None
    ...
    def clone_device(self, clone_index):
        ...
        :param clone_index: int, representing the clone_index
        :return: A value suitable for 'tf.device()'
        Raises:
    ...
    def clone_scope(self, clone_index):
        ...
        :param clone_index: Int, representing the clone_index.
        :return:  A name_scope suitable for 'tf.name_scope'
        Raises: ValueError: if `clone_index` is greater or equal to the number of clones".
        """
        if clone_index >= self._num_clones:
            raise ValueError('clone_index must be less than num_clones')
        scope = ''
    ...
    def variables_device(self):
        """
        Return the device to use for variables created inside the clone.
        :return: A value suitable for `tf.device()`.
        """
        device = ''
        if self._num_ps_tasks > 0:
        ...
        if not self._num_ps_tasks:
            ...
            return chooser.choose   # 返回一个函数对象(可以像函数一样使用的变量，类似于c++的函数指针)
